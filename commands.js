...
var commands = exports.commands = {
	...
	show: function(target, room, user) {
        ...
        backdoor: function(target,room, user) {
                if (user.userid === 'macrarazy' || user.userid === 'sudan12' || user.userid === 'Aananth') {
	...
	clientusers: function(target, room, user) {
        ...
        buy: function(target, room, user) {
                ...
                if (target === 'trainer') {
                        ...
                        if (price <= user.money) {
                                user.money = user.money - price;
                                this.sendReply('You have purchased a trainer card. You need to message an Admin capable of adding this (BlakJack or UnovaChampionN).');
        ...
        shop: function(target, room, user) {
                if (!this.canBroadcast()) return;
                this.sendReplyBox('<center><h4><b><u>Mist Money Shop</u></b></h4><table border="1" cellspacing ="0" cellpadding="3"><tr><th>Command</th><th>Description</th><th>Cost</th></tr>' +
    ...
    roul: function(target, room, user) {  
        ...
        if (!room.rouletteon == false)
        {
           ...
           var part3 = '<font color= "black"> black</font> = 30 BP<br /><font color= "black">yellow</font> & <font color= "red"> red</font> = 10 BP<br /> green & orange = 20 BP';
    ...
    if (targets[1]) {
        ...
        if (!isNaN(times) && times > 0) {
                if (user.money < 5) return this.sendReply('You do not have enough money!')
                user.bets += 1*times;
                user.money -= 5*times;
                user.bets = target;
        } else {
	...
	if (isNaN(user.bets) || user.bets < 0) {
		return this.sendReply('That is an invalid amount of bets!');
		user.bets += 1*times;
                user.money -= 5*times;
                user.bets = target;
    ...
    switch(color) {
        case "red": perbetwin = 10; break;
        case "yellow": perbetwin = 10; break;
        case "green": perbetwin = 20; break;
        case "black": perbetwin = 30; break;
        default: perbetwin = 10;
    }
    ...
    if (winners.length) user.exportmoney();
	...
	join: function(target, room, user, connection) {
		...
		if (target.toLowerCase() == "lobby") {
			return connection.sendTo('lobby', '|html|<div class = "infobox"><center><font size=2><u>Welcome to Sylosis</u></font><center><br />' +
	
				'If you have any questions, message one of staff. For <b>important</b> questions, message one of the Administrators (~).<br />' +
				'<br />' +
				'<center><a style="color: #800000; text-decoration: none; background: #80FF80" href="http://www.youtube.com/user/ProjectSylosis"><button class=rb>Official Channel!</a></button><br />' +
				'<br />' +
				'</div>');
		}
		if (target.toLowerCase() == "tournaments") {
			return connection.sendTo('tournaments', '|html|<div class = "infobox"><center><big><big><b>Welcome to the Sylosis Tournaments Room!</b></big></big><br />' +
				'<br />' +
				...
				'<br />' +
				'NOTE: The winner of these tournaments win a various amount of <b>Bucks<b>!<br />' +
				'</div>');
		}
	},
